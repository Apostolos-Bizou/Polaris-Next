...
export default function SendDocumentsModal({ sd }: { sd: SD }) {
  ...
  const formatText = useCallback((cmd: string) => { previewRef.current?.focus(); document.execCommand(cmd, false, undefined); }, []);
  const insertLink = useCallback(() => { const u = prompt('Enter URL:'); if (u) { previewRef.current?.focus(); document.execCommand('createLink', false, u); } }, []);
  const handleFiles = useCallback((e: React.ChangeEvent<HTMLInputElement>) => { if (e.target.files) sd.setLocalFiles(Array.from(e.target.files)); }, [sd]);
  const sc = (s: string) => { const m: Record<string,string> = { draft:'#95a5a6', sent:'#3498db', accepted:'#27ae60', signed:'#27ae60', pending_signature:'#9b59b6', rejected:'#e74c3c', 'follow-up 1':'#f39c12', 'follow-up 2':'#e67e22', 'follow-up 3':'#e74c3c', followup1:'#f39c12', followup2:'#e67e22', followup3:'#e74c3c' }; return m[(s||'').toLowerCase()] || '#95a5a6'; };
  ...
  return (<div className="sd-modal">
          ...
          {sd.showClientSearch ? (
            ...
            {sd.clientOffers.length>0&&(
              <div className="sd-offers-row">
                ...
                <div className="sd-offers-list">{sd.clientOffers.map((o:any)=>{const d=o.created_at||o.date_created?new Date(o.created_at||o.date_created).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'2-digit'}):'';const c=sc(o.status);const t=parseFloat(o.grand_total_usd||0);return(<div key={o.offer_id} className={`sd-offer-card${sd.selectedOfferId===o.offer_id?' selected':''}`} onClick={()=>sd.selectOffer(o.offer_id,o)}><div className="sd-offer-card-top"><span className="sd-offer-id">{o.offer_id}</span><span className="sd-offer-status" style={{color:c,background:c+'33'}}>{o.status||'draft'}</span></div><div className="sd-offer-card-bot"><span className="sd-offer-date">{d}</span><span className="sd-offer-total">${t.toLocaleString()}</span></div><div className="sd-offer-members">{parseInt(o.total_members||0)} members</div></div>);})}</div>
              </div>
            )}
          </>)}
        </div>
    ...
    <div className="sd-body">
      ...
      <div className="sd-right">
        {/* Email Template */}
        <div className="sd-section"><div className="sd-section-hdr"><span className="sd-section-icon">📝</span><h3>Email Template</h3><button className="sd-manage-btn" onClick={()=>window.open('https://drive.google.com/drive/folders/','_blank')}>📂 Manage Templates</button></div><div className="sd-section-body">
          <select className="sd-select" value={sd.selectedTemplate} onChange={e=>sd.selectTemplate(e.target.value)}>
            <option value="">-- Select Email Template --</option>
            <optgroup label="📄 Document Delivery">{sd.templates.filter(t=>t.category==='Document Delivery').map(t=>(<option key={t.id} value={t.id}>{t.name}</option>))}</optgroup>
            <optgroup label="📋 Follow-up">{sd.templates.filter(t=>t.category==='Follow-up').map(t=>(<option key={t.id} value={t.id}>{t.name}</option>))}</optgroup>
            {sd.templates.filter(t=>!['Document Delivery','Follow-up'].includes(t.category)).length>0&&<optgroup label="📁 Other">{sd.templates.filter(t=>!['Document Delivery','Follow-up'].includes(t.category)).map(t=>(<option key={t.id} value={t.id}>{t.name}</option>))}</optgroup>}
          </select>
        </div></div>

        {/* Recipients */}
        <div className="sd-section"><div className="sd-section-hdr"><span className="sd-section-icon">👥</span><h3>Recipients</h3></div><div className="sd-section-body">
          <div className="sd-recipients-box">
            {sd.recipients.length===0?<p className="sd-placeholder">🔍 Search and select a client first</p>:sd.recipients.map((r,i)=>(<div key={i} className="sd-recip-row"><input type="checkbox" checked={r.checked} onChange={()=>sd.toggleRecipient(i)}/><div className="sd-recip-info"><div className="sd-recip-name">{r.name}</div><div className="sd-recip-email">{r.email}</div></div><span className="sd-recip-role">{r.role}</span></div>))}
          </div>
          <div className="sd-role-btns">{['all','Primary','Finance','Operations','Marketing','Develop','Legal','clear'].map(role=>(<button key={role} className="sd-role-btn" onClick={()=>sd.selectRecipientsByRole(role)}>{role==='all'?'☑️ All':role==='clear'?'✖️ Clear':role==='Primary'?'👤 Primary':role==='Finance'?'💰 Finance':role==='Operations'?'🔧 Operations':role==='Marketing'?'📣 Marketing':role==='Develop'?'💻 Develop':'⚖️ Legal'}</button>))}</div>
        </div></div>

        {/* Email Body */}
        <div className="sd-section sd-email-section"><div className="sd-section-hdr"><span className="sd-section-icon">✏️</span><h3>Email Body</h3><span className="sd-editable-badge">✎ Editable</span></div><div className="sd-section-body">
          <div className="sd-toolbar">
            <button onClick={()=>formatText('bold')}><strong>B</strong></button><button onClick={()=>formatText('italic')}><em>I</em></button><button onClick={()=>formatText('underline')} style={{textDecoration:'underline'}}>U</button><span className="sd-toolbar-sep"/><button onClick={()=>formatText('insertUnorderedList')}>• List</button><button onClick={()=>formatText('insertOrderedList')}>1. List</button><span className="sd-toolbar-sep"/><button onClick={insertLink}>🔗 Link</button><span className="sd-toolbar-sep"/><button className="sd-reset-btn" onClick={sd.resetTemplate}>↺ Reset</button>
          </div>
          <div ref={previewRef} className="sd-preview" contentEditable suppressContentEditableWarning dangerouslySetInnerHTML={{__html:sd.emailBody||'<div class="sd-preview-placeholder">Select a template to see preview...</div>'}} onBlur={e=>sd.setEmailBody(e.currentTarget.innerHTML)}/>
          <div className="sd-tip">💡 Click inside to edit. Use toolbar for formatting.</div>
        </div></div>

        {sd.status&&<div className={`sd-status ${sd.status.type}`}>{sd.status.msg}</div>}
      </div>
    </div>
    ...
    <div className="sd-footer">
      ...
      <div className="sd-footer-actions">
        <button className="sd-btn preview" onClick={()=>alert('Preview coming soon')}>👁️ Preview</button>
        <button className="sd-btn draft" onClick={sd.saveDraft} disabled={sd.loading}>💾 Draft</button>
...
.sd-dropdown{position:absolute;z-index:100;width:100%;max-height:250px;overflow-y:auto;background:rgba(10,22,40,0.8);border:2px solid #d4af37;border-radius:12px;margin-top:5px;box-shadow:0 10px 40px rgba(0,0,0,0.5)}
.sd-dropdown-item{padding:12px 18px;cursor:pointer;border-bottom:1px solid #0d1f2d;transition:background 0.2s}.sd-dropdown-item:hover{background:rgba(212,175,67,0.2)}
...
.sd-offer-card{background:linear-gradient(135deg,#1a2332,#243447);border:2px solid transparent;border-radius:10px;padding:10px 14px;cursor:pointer;min-width:180px;transition:all 0.2s}
.sd-offer-card:hover{border-color:#d4af37;transform:translateY(-2px)}.sd-offer-card.selected{border-color:#d4af37;background:linear-gradient(135deg,#2a3a4a,#344a5a)}
...
.sd-body{flex:1;overflow-y:auto;padding:25px 30px;display:grid;grid-template-columns:1fr 1fr;gap:30px;background:rgba(10,22,40,0.8)}.sd-left,.sd-right{display:flex;flex-direction:column;gap:20px}
...
.sd-select{width:100%;padding:12px 14px;background:rgba(10,22,40,0.8);border:1px solid #3182ce;border-radius:8px;color:white;font-size:0.9rem;cursor:pointer;outline:none}.sd-select:focus{border-color:#d4af37}
...
.sd-manage-btn{margin-left:auto;padding:6px 12px;background:linear-gradient(135deg,rgba(212,175,55,0.2),rgba(212,175,55,0.1));border:1px solid #d4af37;border-radius:6px;color:#d4af37;font-size:0.75rem;cursor:pointer}.sd-manage-btn:hover{background:rgba(212,175,55,0.3)}
.sd-recipients-box{background:rgba(10,22,40,0.8);border:2px solid rgba(45,80,112,0.35);border-radius:12px;padding:16px;max-height:200px;overflow-y:auto}
.sd-placeholder{color:#7aa0c0;text-align:center;padding:20px;margin:0}
.sd-recip-row{display:flex;align-items:center;gap:12px;padding:10px;border-radius:8px;transition:background 0.2s}.sd-recip-row:hover{background:rgba(255,255,255,0.05)}
.sd-recip-row input[type="checkbox"]{width:20px;height:20px;accent-color:#1976D2}.sd-recip-info{flex:1}.sd-recip-name{color:white;font-weight:500}.sd-recip-email{color:#7aa0c0;font-size:0.85rem}
.sd-recip-role{font-size:0.7rem;padding:4px 10px;border-radius:20px;background:rgba(45,80,112,0.35);color:#b8d4e8}
.sd-role-btns{display:flex;gap:10px;margin-top:12px;flex-wrap:wrap}
.sd-role-btn{padding:8px 16px;background:rgba(45,80,112,0.35);border:none;border-radius:20px;color:white;cursor:pointer;font-size:0.85rem;transition:all 0.2s}.sd-role-btn:hover{background:#5a6a7a;transform:translateY(-1px)}
.sd-email-section{flex:1}.sd-editable-badge{margin-left:auto;font-size:0.75rem;color:#4CAF50;background:rgba(76,175,80,0.2);padding:4px 12px;border-radius:12px}
.sd-toolbar{display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;padding:10px;background:rgba(10,22,40,0.8);border-radius:10px}
.sd-toolbar button{padding:8px 12px;background:rgba(45,80,112,0.35);border:none;border-radius:6px;color:white;cursor:pointer;font-size:0.9rem;transition:all 0.2s}.sd-toolbar button:hover{background:#5a6a7a}
.sd-toolbar-sep{width:1px;background:rgba(45,80,112,0.35);margin:0 5px}.sd-reset-btn{background:#e74c3c!important}.sd-reset-btn:hover{background:#c0392b!important}
.sd-preview{background:#fff;border-radius:12px;padding:20px;color:#333;min-height:200px;max-height:250px;overflow-y:auto;outline:none;border:2px solid transparent;transition:border-color 0.3s;cursor:text;font-family:Arial,sans-serif;font-size:14px;line-height:1.6}
.sd-preview:focus{border-color:#1976D2}.sd-preview-placeholder{text-align:center;color:#999;padding:30px}.sd-tip{margin-top:8px;font-size:0.75rem;color:#7aa0c0}
.sd-status{padding:12px 20px;border-radius:10px;text-align:center;margin-top:15px}.sd-status.success{background:rgba(76,175,80,0.2);color:#4CAF50}.sd-status.error{background:rgba(231,76,60,0.2);color:#e74c3c}.sd-status.loading{background:rgba(25,118,210,0.2);color:#1976D2}
.sd-footer{background:#0d1f2d;padding:12px 20px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid rgba(45,80,112,0.35);flex-shrink:0;gap:15px}
...
.sd-no-docs{color:#7aa0c0;font-size:0.8rem}.sd-footer-actions{display:flex;gap:8px}
.sd-btn{padding:8px 16px;border:none;border-radius:25px;font-family:'Montserrat',sans-serif;font-weight:600;cursor:pointer;transition:all 0.3s;font-size:0.85rem;display:flex;align-items:center;gap:8px}
.sd-btn:disabled{opacity:0.5;cursor:not-allowed}.sd-btn.preview{background:rgba(45,80,112,0.35);color:white}.sd-btn.preview:hover:not(:disabled){background:#5a6578;transform:translateY(-2px)}
...
.sd-btn.send{background:linear-gradient(135deg,#27ae60,#2ecc71);color:white;padding:8px 20px;font-size:0.9rem}.sd-btn.send:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 5px 20px rgba(39,174,96,0.4)}
@media(max-width:1024px){.sd-body{grid-template-columns:1fr;padding:15px}.sd-prog-grid{grid-template-columns:repeat(3,1fr)}.sd-header{padding:15px 20px}.sd-title{font-size:1.3rem}.sd-footer{flex-direction:column;gap:15px}.sd-footer-actions{width:100%;flex-direction:column}.sd-btn{width:100%;justify-content:center}}
